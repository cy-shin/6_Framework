...
// 기존에 존재하다가 없어진 삭제된 이미지의 순서를 기록
const deleteSet = new Set(); // SET : 중복 x 순서 x

// 미리보기와 관련된 모든 요소의 개수는 5개로 동일
// == 인덱스 번호를 통해서 하나의 그룹을 지정할 수 있다
...
for(let i = 0; i < inputImage.length; i++) {
    ...
    inputImage[i].addEventListener("change", event => {
        ...
        // files에 어떤 파일이 업로드 되었는지 배열 형태로 남아있음
...
function updateValidate(){
    ...
    if(boardContent.value.trim().length == 0) { 
     ...
     // form태그 내부 input 태그로 추가하여
     // 같이 제출하게 만들기

     // Array.from(deleteSet) -> deleteSet을 배열로 변환
     // JS배열 특징 : 배열을 HTML 요소 또는 console에 출력하면 
     // 요소, 요소, 요소 형태의 문자열로 반환
