...
@RestController // 요청에 따른 응답이 모두 값 자체인 컨트롤러(Controller 및 ResponseBody 전체 적용) + bean 등록 기능
				// forward, response 불가
...
public class CommentController {
	...
	public String selectCommentList(int boardNo) {
		...
		return new Gson().toJson(rList); // JSON 형태로 반환(GSON 라이브러리 이용)
	}
	...
	// 댓글 수정
	// Get 방식 사용 시, 내용이 길면 주소에 값이 다 담기지 않을 수 있음 
	@PostMapping("/update")
