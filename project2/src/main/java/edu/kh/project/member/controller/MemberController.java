...
//	private MemberService service = new MemberServiceImpl();
	...
	public String login(/*@ModelAttribute*/ Member inputMember, 
		...
		if(loginMember != null) {
			...
			// @SessionAttributes("Key값") 클래스 위에 추가
			// -> session scope로 변환
			...
			// 쿠키가 사용될 경로
			cookie.setPath("/"); // localhost 밑에 모든 주소에서 사용함
			
			// 생성된 쿠키를 응답 객체에 담아서 클라이언트로 전달
	...
	public String signUp(/*@ModelAttribute*/ Member inputMember /*커맨드 객체*/,
							String[] memberAddress /*name 속성값이 memberAddress인 값을 배열로 반환*/,
							RedirectAttributes ra,
							@RequestHeader("referer") String referer
							
			) {
		// Member inputMember와 String[] memberAddress 양 쪽 다에 입력값이 저장됨
	 ...
	 * 			-> 메서드 단위로 처리
	 * 			-> 메서드 단위로 작성을 해야해서 불편함
	 * 
	 * 2순위 : @ExceptionHandelr 어노테이션(별도 처리가 필요한 경우 작성)
	 * 			-> 클래스 단위로 처리
	 * 			- 하나의 컨트롤러에서 발생하는 예외를 하나의 메서드에 모아서 처리
	 * 
	 * 3순위 : @ControllerAdvice 어노테이션(기본값)
	 * 			-> 전역(웹 애플리케이션)에서 발생하는 예외를 모아서 처리
	 * 			- 별도의 클래스로 작성함
	 * 
	 * 권장 : 3순위 @ControllerAdvice를 기본값으로 사용하고, 필요 시 @ExceptionHandeler 어노테이션을 활용
	 * 
	 **/
	
	// MemberController 에서 발생하는 예외를 모아서 처리
